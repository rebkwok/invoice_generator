...
class InvoiceGenerator:
    ...
    def generate(self, inv_date, inv_num):
        ...
        inv_num = f"{str(inv_num).zfill(int(self.config.get('invoice_num_length', 4)))}{self.config.get('invoice_num_suffix', '')}"
        ...
        page_layout.add(Paragraph("INVOICE", horizontal_alignment=Alignment.RIGHT, font_size=Decimal(16), font_color=HexColor("a19f9a")))
        ...
        filename = Path(output_dir) / f"{self.config['invoice_filename_prefix']}_{inv_num}_{inv_date}.pdf"
    ...
    def _build_invoice_information(self, inv_date, inv_num):  
        ...
        table_001.add(Paragraph(self.config["bill_for"], horizontal_alignment=Alignment.RIGHT))
    ...
    def _build_itemized_description_table(self):
        ...
        table_001.add(
            TableCell(
                ...
                background_color=even_color, border_bottom=False
            )
        )
        table_001.add(TableCell(Paragraph(f"£{self.config['amount']}", horizontal_alignment=Alignment.RIGHT), background_color=even_color, border_bottom=False))
                    
        # Optionally add some empty rowsfor styling purposes 
        for _ in range(0, 8):  
            table_001.add(TableCell(Paragraph(" "), background_color=even_color, border_top=False, border_bottom=False))
        ...
        table_001.add(TableCell(Paragraph(f"£{self.config['amount']}", horizontal_alignment=Alignment.RIGHT), background_color=HexColor("e8e7e3")))  
    ...
    if inv_type is not None:
        if inv_type not in config:
            raise click.ClickException(
                f"Invoice type '{inv_type}' not found in config file at {config_path}; options are {', '.join(config.sections())}"
            )

        return {
            **config["DEFAULT"], **config[inv_type]
        }
    else:
        return config["DEFAULT"]

@click.command()
@click.option('--config', "-c", default="config.ini", type=Path, help="Location of config file; defaults to 'config.ini' in current directory")
@click.option('--invoice-type', "-t", help="Invoice type; must correspond to a section in config.ini")
@click.option('--invoice-num', "-i", type=int, default=1, help="Invoice number to start at; defaults to 1")
@click.option("--dates", "-d", multiple=True, help="Invoice dates, in YYYYMMDD format")
